...
void SoftPWMSet(int8_t pin, uint8_t value, bool hardset)
{
...
uint8_t SoftPWMGetRaw(int8_t pin)
{
  for (uint8_t i = 0; i < SOFTPWM_MAXCHANNELS; i++)
  {
    if (_softpwm_channels[i].pin == pin)
    {
      return _softpwm_channels[i].checkval;
    }
  }
  return 0;
}
uint8_t SoftPWMGet(int8_t pin)
{
  for (uint8_t i = 0; i < SOFTPWM_MAXCHANNELS; i++)
  {
    if (_softpwm_channels[i].pin == pin)
    {
      return _softpwm_channels[i].pwmvalue;
    }
  }
  return 0;
}

bool SoftPWMisFading(int8_t pin)
{
  for (uint8_t i = 0; i < SOFTPWM_MAXCHANNELS; i++)
  {
    if (_softpwm_channels[i].pin == pin)
    {
      return _softpwm_channels[i].checkval != _softpwm_channels[i].pwmvalue;
    }
  }
  return false;
}
